...
`bbcat-gtk` is a GTK4 viewer for the
...
It has a file chooser, supports opening a file from the command line or desktop,
and plays animations at `bbcat`'s default speed. The `×1` and `×2` buttons
select bbcat's integer PNG rendering scale. A normal window follows the rendered
content size up to the available monitor space.
...
The Makefile installs the release binary, desktop entry, application icon, and
the MIME definitions used for file associations. To install them for the
current user:
...
The desktop entry associates bbcat with ANSI (`.ans`, `.asc`, `.diz`), NFO,
DarkDraw (`.ddw`), ArtWorx (`.adf`), RIPscrip (`.rip`), and XBin (`.xb`, `.xbin`)
artwork. The `update-mime-database` and `update-desktop-database`
